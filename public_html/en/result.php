...
function get_right_answer($counter, $input, $type, $cat){
    $r_answer = "SELECT Answer from right_answers_en AS ra, fragenpool_en AS f WHERE f.RA_ID = ra.RA_ID AND ra.RA_ID = ".$counter;
    $r2_answer = "SELECT Answer2 from right_answers_en AS ra, fragenpool_en AS f WHERE f.RA_ID = ra.RA_ID AND ra.RA_ID = ".$counter;
    ...
    if($calculation != $antwort_result){
        echo "Sorry, your answer $input ($calculation) is wrong.";
        echo "<br>";
        echo "The correct answer is: $antwort_result ($antwort2_result)";
        $_SESSION['answer'] = 0;
    }
    elseif($calculation == $antwort_result){
        echo "Your answer $input ($calculation) is correct!";
        //echo "<br>The correct answer is: $antwort_result ($antwort2_result)";
        //echo "<br>query =     $r_answer";
        $_SESSION['answer'] = 1;
    }
}
...
function get_question($counter){
    $sql_results = "SELECT Frage from fragenpool_en where F_ID = $counter";
...
if($_POST['rnd'] == 'Sonder') $result = mysql_query("SELECT F_ID FROM fragenpool_en where
												Kategorie = 'Laplace' OR Kategorie = 'Permutation' OR Kategorie = 'Summenregel'
												OR Kategorie = 'Produktregel' ORDER BY RAND() LIMIT 5");

else if($_POST['rnd'] == 'VariKombi') $result = mysql_query("SELECT F_ID FROM fragenpool_en where
...
function get_type($qid){
    $sql_results = "SELECT Art from fragenpool_en where F_ID = $qid";
    $query = mysql_query($sql_results);
...
function get_category($qid){
    $sql_results = "SELECT Kategorie from fragenpool_en where F_ID = $qid";
    $query = mysql_query($sql_results);
...
function get_img($id, $category, $type){
    $sql_results = "SELECT img from fragenpool_en where Kategorie = '$category' AND Art = '$type' AND F_ID = ".$id;
    $query = mysql_query($sql_results);
...
function get_tips($counter){
    $tt = mysql_fetch_row(mysql_query("SELECT Tipp1,Tipp2,Tipp3,Tipp4,Tipp5 from tooltipps_en,fragenpool_en where
									  tooltipps_en.TT_ID = fragenpool_en.TT_ID AND fragenpool_en.F_ID = ". $counter));
...
function get_downloads($qid, $count){
    ...
    if($qid == 38 OR $qid == 39) {
        echo '
                       The Excel worksheet offers for the birthday questions 2 and 3 in sheet „Calculation“ the summary of the calculation and in sheet „Simulation of all possibilities“ a big table where the computer works through all possible cases and counts the events for each category. This allows an independent control and confirmation of the calculation.
                      <br /><br /> Solutions: <br />
                      <a href="downloads/birthday_weekday.xlsx" download>Excel sheet birthday_weekday.xlsx</a><br /><br />';
    }
    else echo "Solutions: <br />";
    $i = 1;
    //Laden aller dateien in dem Ordner
    foreach($directory as $value){
        echo "<a href='$path/$value' download>Handwritten solution to Question  $count	 </a>";
        $i++;
    }
