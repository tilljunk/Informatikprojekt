...
function get_right_answer($counter, $input, $type, $cat){
    ...
    if($calculation != $antwort_result){
        ...
        echo "<br>";
        echo "Die richtige Antwort lautet $antwort_result.";
        $_SESSION['answer'] = 0;
    }
...
function get_tips($counter){
    $tt = mysql_fetch_row(mysql_query("SELECT Tipp1,Tipp2 from tooltipps,fragenpool where
									  tooltipps.TT_ID = fragenpool.TT_ID AND fragenpool.F_ID = $counter"));
...
function get_downloads($qid, $count){
    ...
    if($qid == 38) {
        echo '
                       Die Excel-Mappe bietet für die Geburtstagsaufgaben 2-4 im Blatt „Rechnung“ das Ganze zusammengefasst und im Blatt „Simulation aller Fälle“ noch eine anschauliche Tabelle mit allen Fällen und ermöglicht damit eine unabhängige Kontrolle und Bestätigung der Rechnung.
                      <br /><br /> Lösungen: <br />
    ...
    foreach($directory as $value){
        echo "<a href='$path/$value' download>$i. Download der $count. Aufgabe</a>";
        $i++;
    }
...
<span id="tips"><?php echo get_tips($qid)[0]; ?></span>
<span id="tips_content"><?php echo get_tips($qid)[2]; ?></span>
